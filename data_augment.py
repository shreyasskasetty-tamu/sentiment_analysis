...
if __name__ == '__main__':
    if os.path.exists(config.PREPROCESSED_DATA_PATH):
        ...
        preprocessed_dataset = dataset.load_preprocessed_data(config.PREPROCESSED_DATA_PATH)
        ...
        dataset.save_preprocessed_data(preprocessed_dataset, config.PREPROCESSED_DATA_PATH)
    ...
    downsampled_positive_reviews = preprocessed_dataset[preprocessed_dataset['sentiment'] == 2].sample(n=50000, random_state=config.RANDOM_STATE)
    ...
    dataset.save_preprocessed_data(balanced_data,config.BALANCED_DATASET_PATH)